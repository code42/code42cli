...
def test_reset_calls_connection_drop_table_query(test_store, execute_function):
    test_store.reset()
    execute_function.assert_called_once_with("DROP TABLE siem_checkpoint")


def test_init_cursor_store_when_not_given_db_file_path_used_expected_path(mocker):
...
def test_init_cursor_store_when_given_db_file_path_uses_given_path(mocker):
    mock_connect_function = mocker.patch("sqlite3.connect")
