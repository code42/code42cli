...
class TestAlertCursorStore(object):
    ...
    def test_replace_stored_cursor_timestamp_writes_to_expected_file(self, mock_open):
        store = AlertCursorStore(PROFILE_NAME)
        store.replace("checkpointname", 123)

    def test_clean_calls_remove_on_each_checkpoint(self):
        store = AlertCursorStore(PROFILE_NAME)
        store.clean(CURSOR_NAME)
...
class TestFileEventCursorStore(object):
    def test_get_stored_cursor_timestamp_returns_expected_timestamp(self):
        store = AlertCursorStore(PROFILE_NAME)
        store.get(CURSOR_NAME)

    def test_replace_stored_cursor_timestamp_writes_to_expected_file(self):
        store = AlertCursorStore(PROFILE_NAME)
        store.replace("checkpointname", 123)
