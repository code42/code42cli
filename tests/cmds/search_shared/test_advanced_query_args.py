...
def test_exit_if_advanced_query_provided_incompatible_args(file_event_namespace, alert_namespace):
    ...
    with pytest.raises(SystemExit):
        exit_if_mutually_exclusive_args_used_together(
            file_event_namespace, list(create_incompatible_search_args().keys())
        )
    ...
    with pytest.raises(SystemExit):
        exit_if_mutually_exclusive_args_used_together(
            alert_namespace, list(create_incompatible_search_args().keys())
        )
