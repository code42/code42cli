...
from c42sec.args import AEDArgs

from c42sec import main_old
...
def mock_store(mocker):
    store = mocker.patch("c42sec.aed.main.AEDCursorStore.__init__")
...
def mock_args_getter(mocker):
    return mocker.patch("c42sec.aed.args.get_args")


@pytest.fixture
def mock_password_getter(mocker):
    mock = mocker.patch("c42sec.common.util.get_stored_password")
...
def mock_password_deleter(mocker):
    return mocker.patch("c42sec.common.util.delete_stored_password")


@pytest.fixture
def mock_logger(mocker):
    return mocker.patch("c42sec.common.util.get_logger")


@pytest.fixture
def mock_error_logger(mocker):
    return mocker.patch("c42sec.common.util.get_error_logger")


@pytest.fixture
def mock_cursor_reset_function(mocker):
    return mocker.patch("c42sec.aed.main.AEDCursorStore.reset")
...
def test_main_when_reset_password_is_true_calls_delete_password(patches):
    ...
    main_old.main()
    patches.delete_password.assert_called_once_with(main_old._SERVICE_NAME, expected_username)
