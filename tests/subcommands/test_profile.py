...
def username_setter(mocker):
    return mocker.patch("c42sec._internal.profile.config.set_username")


@pytest.fixture
def mark_as_set_function(mocker):
    return mocker.patch("c42sec._internal.profile.config.mark_as_set")


@pytest.fixture
def authority_url_setter(mocker):
    return mocker.patch("c42sec._internal.profile.config.set_authority_url")


@pytest.fixture
def ignore_ssl_errors_setter(mocker):
    return mocker.patch("c42sec._internal.profile.config.set_ignore_ssl_errors")


@pytest.fixture
def password_setter(mocker):
    return mocker.patch("c42sec._internal.profile.password.set_password")


@pytest.fixture
def password_getter(mocker):
    return mocker.patch("c42sec._internal.profile.password.get_password")


@pytest.fixture
def profile_not_set_state(mocker):
    profile_verifier = mocker.patch("c42sec._internal.profile.config.profile_has_been_set")
