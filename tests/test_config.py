...
class TestConfigAccessor(object):
    def test_get_profile_when_profile_does_not_exist_raises(self, mock_config_parser, mock_logger):
        mock_config_parser.sections.return_value = [_INTERNAL]
        accessor = ConfigAccessor(mock_config_parser, logger=mock_logger)
        with pytest.raises(NoConfigProfileError):
            accessor.get_profile("Profile Name that does not exist")

    def test_get_profile_when_profile_has_default_name_raises(
        self, mock_config_parser, mock_logger
    ):
        mock_config_parser.sections.return_value = [_INTERNAL]
        accessor = ConfigAccessor(mock_config_parser, logger=mock_logger)
        with pytest.raises(NoConfigProfileError):
    ...
    def test_get_profile_returns_expected_profile(self, mock_config_parser, mock_logger):
    ...
    def test_get_all_profiles_excludes_internal_section(self, mock_config_parser, mock_logger):
        mock_config_parser.sections.return_value = [
            _TEST_PROFILE_NAME,
            _INTERNAL,
            _TEST_SECOND_PROFILE_NAME,
    ...
    def test_get_all_profiles_returns_profiles_with_expected_values(
        self, config_parser_for_multiple_profiles, mock_logger
    ):
        accessor = ConfigAccessor(config_parser_for_multiple_profiles, logger=mock_logger)
        profiles = accessor.get_all_profiles()
    ...
    def test_switch_default_profile_saves(
        self, config_parser_for_multiple_profiles, mock_saver, mock_logger
    ):
        accessor = ConfigAccessor(config_parser_for_multiple_profiles, logger=mock_logger)
        accessor.switch_default_profile(_TEST_SECOND_PROFILE_NAME)
    ...
    def test_switch_default_profile_outputs_confirmation(
        ...
        expected = "{} has been set as the default profile.".format(_TEST_SECOND_PROFILE_NAME)
        mock_logger.info.assert_called_once_with(expected)

    def test_create_profile_when_given_default_name_does_not_create(
        self, config_parser_for_create, mock_logger
    ):
        accessor = ConfigAccessor(config_parser_for_create, logger=mock_logger)
    ...
    def test_create_profile_when_no_default_profile_sets_default(
        self, mocker, config_parser_for_create, mock_saver, mock_logger
    ):
    ...
    def test_create_saves(self, config_parser_for_create, mock_saver, mock_logger):
        mock_profile = create_mock_profile_object(_TEST_PROFILE_NAME)
        mock_internal = create_internal_object(False)
        setup_parser_one_profile(mock_profile, mock_internal, config_parser_for_create)
        accessor = ConfigAccessor(config_parser_for_create, logger=mock_logger)
    ...
    def test_create_profile_outputs_confirmation(
        self, config_parser_for_create, mock_saver, mock_logger
    ):
        mock_profile = create_mock_profile_object(_TEST_PROFILE_NAME)
        mock_internal = create_internal_object(False)
        setup_parser_one_profile(mock_profile, mock_internal, config_parser_for_create)
        ...
        accessor.create_profile(_TEST_PROFILE_NAME, "example.com", "bar", False)
        expected = u"Successfully saved profile '{}'.".format(_TEST_PROFILE_NAME)
        assert mock_logger.info.call_args_list[1][0][0] == expected
    ...
    def test_update_profile_updates_profile(self, config_parser_for_multiple_profiles, mock_logger):
        accessor = ConfigAccessor(config_parser_for_multiple_profiles, logger=mock_logger)
        address = "NEW ADDRESS"
