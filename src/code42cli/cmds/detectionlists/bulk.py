...
class BulkRiskTagCommandType(BulkCommandType):
    ADD_RISK_TAG = u"add-risk-tags"
    REMOVE_RISK_TAG = u"remove-risk-tags"

    def __iter__(self):
        parent_items = list(super(BulkRiskTagCommandType).__iter__())
...
class BulkHighRiskEmployee(BulkDetectionList):
    ...
    def __init__(self):
        super(BulkHighRiskEmployee).__init__()
        self.type = BulkRiskTagCommandType

    def get_handler(self, handlers, cmd):
        handler = super(BulkHighRiskEmployee).get_handler(handlers, cmd)
        if not handler:
