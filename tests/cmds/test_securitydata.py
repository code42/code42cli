...
def test_non_exposure_only_query_with_checkpoint_does_not_send_empty_filter_list(runner, cli_state, mock_file_event_checkpoint, mocker):
    mock_file_event_checkpoint.get.return_value = "event_1234"
    mock_get_all_file_events = mocker.patch("code42cli.cmds.securitydata._get_all_file_events")
    mock_get_all_file_events.return_value = []
    result = runner.invoke(cli, ["security-data", "search", "--include-non-exposure", "-c", "checkpoint"])
